...
public class MassekugelnNext extends GraphicsProgram {
	...
	// get kreisGrav 4
	// Bekommt Arraylist von dimensionen die mit einander ableitungsverknüpft seien
	// können
	// Dimensionarray: Höhe=Raumdimensionen; Breite=Ableitungen
	// Teilstrukturen mit halben durchmesser -> Vierfache dichte nötig für gleichbleibende gravitative Fluchtgeschwindigkeit
	...
	public void getKreisGrav4(long it) {
		...
		for (int i = 0; i < it; i++) {
			...
			// Gehe dimensionsarray von a und b durch und addiere jeweils das quadrat der
			// differenz
			double swapA = 0;
			double swapB = 0;

			double distanceQ = 0;
			for (int ab = 0; ab < a.dimensionsarray[0].length; ab++) {
				for (int dim = 0; dim < a.dimensionsarray.length; dim++) {
					distanceQ = distanceQ + (a.dimensionsarray[dim][ab] - b.dimensionsarray[dim][ab])
							* (a.dimensionsarray[dim][ab] - b.dimensionsarray[dim][ab]);
					swapA += (a.dimensionsarray[dim][ab] / distanceQ);
					swapB += (b.dimensionsarray[dim][ab] / distanceQ);
				}
			}
			...
			if (swapB < 0)
				swapB = -swapB;
			// if(swapA<1)swapA=Math.random();
			// if(swapB<1)swapB=Math.random();

//			if (swapA > distanceQ * 2/a.masse/b.m) {
//				while (swapA * a.masse * b.masse > distanceQ * 2) {
//
//					swapA *= Math.random();
//
//				}
//			}
//			if (swapB > 1) {
//				while (swapB * a.masse * b.masse > distanceQ * 2) {
//
//					swapB *= Math.random();
//
//				}
//			}
			...
			for (int ab = 0; ab < a.dimensionsarray[0].length; ab++) {
				for (int dim = 0; dim < a.dimensionsarray.length; dim++) {
					a.dimensionsarray[dim][ab] = a.dimensionsarray[dim][ab] - a.dimensionsarray[dim][ab] / distanceQ;
					b.dimensionsarray[dim][ab] = b.dimensionsarray[dim][ab]
							+ a.dimensionsarray[dim][ab] / distanceQ * a.masse;
					b.dimensionsarray[dim][ab] = b.dimensionsarray[dim][ab] - b.dimensionsarray[dim][ab] / distanceQ;
					a.dimensionsarray[dim][ab] = a.dimensionsarray[dim][ab]
							+ b.dimensionsarray[dim][ab] / distanceQ * b.masse;
					a.dimmove();
					b.dimmove();
				}
			}
	...
	public void konstStrukt(double maxx, double maxy, double maxz, double maxMass) {
		...
		for (int i = 0; i < strukt.length; i++) {
			...
			if (i < 1) {
				...
				posY = 1000 * Math.random();
				posZ = 1000 * Math.random();
				dx = Math.random();
				dy = Math.random();
				dz = Math.random();
			...
			strukt[i].dimensionsarray = new double[3][2];
			strukt[i].dimensionsarray[0][0] = posX;
			strukt[i].dimensionsarray[1][0] = posY;
			strukt[i].dimensionsarray[2][0] = posZ;
			strukt[i].dimensionsarray[0][1] = dx;
			strukt[i].dimensionsarray[1][1] = dy;
			strukt[i].dimensionsarray[2][1] = dz;
	...
	public void konstStrukt2(double maxx, double maxy, double maxz, double maxMass) {
		...
		for (int i = 0; i < strukt.length; i++) {
			...
			double dx = Math.random() * maxx;
			double dy = Math.random() * maxy;
			double dz = Math.random() * maxz;
	...
	public static void main(String[] args) {
		new MassekugelnNext().start(args);
	}
