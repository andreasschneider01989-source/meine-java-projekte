...
public class Massekugeln3Slow extends GraphicsProgram {
...
//			objekte[i][0] = 100.0;
	...
	public int getKreisalt(double teilnummer) {
		for (int i = 0; i < kreise.length; i++) {
			if (teilnummer <= 0)
				return i;
	...
	//Unteraufteilung der Massen
		public int getKreis(double teilnummer) {
			...
			while (true) {
				if (teilnummer <= 0)
					{return count%objekte.length;}
				teilnummer -= objekte[count%objekte.length][0];
...
//	}
	...
	public void getKreisGravAuto(int konst) {
		long time = System.currentTimeMillis();
...
//					double teilnummer = (int) (Math.random() * 250000);
//
...
//					int j = kreisNrB; // 100 Kreise
					...
					int a =getKreis(teilnummer);
					teilnummer = (int) (Math.random() * 250000);
					int b =getKreis(teilnummer);

					double MA = objekte[a][0];
					double MB = objekte[b][0];

					double Dist1 = (objekte[a][1] - objekte[b][1]) * (objekte[a][1] - objekte[b][1]);
					double Dist2 = (objekte[a][2] - objekte[b][2]) * (objekte[a][2] - objekte[b][2]);
					double Dist3 = (objekte[a][3] - objekte[b][3]) * (objekte[a][3] - objekte[b][3]);
					double dist = Dist1 + Dist2 + Dist3;
					if(dist<0.00001)dist=0.00001;

					for (int i = 1; i < objekte[0].length; i++) {

						double swapA = objekte[a][i] * Math.random();
						double swapB = objekte[b][i] * Math.random();
...
//
//					int kreisNrA = getKreis(teilnummer); // 100 Kreise
					double teilnummer = (int) (Math.random() * 200000);
//					int kreisNrB = getKreis(teilnummer); // 100 Kreise
//					
//					
//					int j = kreisNrB; // 100 Kreise
					...
					int a =getKreis(teilnummer);
					 teilnummer = (int) (Math.random() * 200000);
					int b =getKreis(teilnummer);
	...
	public void drawKreise() {
		...
		for (int i = 0; i < objekte.length; i++) {
			...
			;
			 ...
			 //To Get Kreis --- Mehrere Objekte als kleinste einzelne Masse sollten auf den Random Ort(x,y) eines 
	...
	public static void main(String[] args) {
		new Massekugeln3Slow().start(args);
	}
