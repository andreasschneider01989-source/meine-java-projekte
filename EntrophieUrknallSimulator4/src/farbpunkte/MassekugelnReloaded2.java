...
public class MassekugelnReloaded2 extends GraphicsProgram {
	...
	public void konstStrukt(double maxx, double maxy, double maxz, double maxMass) {
		for (int i = 0; i < strukt.length; i++) {
			...
			double masse=maxMass*Math.random()-maxMass*Math.random();
			
			if(masse>0 && masse<1)masse=1;
...
//		if(ButtonCount==null)ButtonCount=new double[1][15];
	...
	public void run() {
		...
		// initt();
		addMouseListeners();
		konstStrukt(10000, 10000, 10000, 10);
		drawKreisePunkte2(0.2);
		// urknall();
	}

	public static void main(String[] args) {
		new MassekugelnReloaded2().start(args);
	}
