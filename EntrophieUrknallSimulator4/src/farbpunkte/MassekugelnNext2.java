...
public class MassekugelnNext2 extends GraphicsProgram {
...
//			double dismul = 5;
			...
			if (notme) {
				double swapAX = a.dx / distQ1;
				double swapBX = b.dx / distQ1;
				double swapAY = a.dy / distQ2;
				double swapBY = b.dy / distQ2;
				double swapAZ = a.dz / distQ3;
				double swapBZ = b.dz / distQ3;
				...
				a.dx = a.dx - swapAX * b.masse/Math.max(1,Math.pow(a.dx-b.dx,2));
				a.dy = a.dy - swapAY * b.masse/Math.max(1,Math.pow(a.dy-b.dy,2));
				a.dz = a.dz - swapAZ * b.masse/Math.max(1,Math.pow(a.dz-b.dz,2));
				b.dx = b.dx + swapAX * a.masse/Math.max(1,Math.pow(a.dx-b.dx,2));
				b.dy = b.dy + swapAY * a.masse/Math.max(1,Math.pow(a.dy-b.dy,2));
				b.dz = b.dz + swapAZ * a.masse/Math.max(1,Math.pow(a.dz-b.dz,2));

				b.dx = b.dx - swapBX * a.masse/Math.max(1,Math.pow(a.dx-b.dx,2));
				b.dy = b.dy - swapBY * a.masse/Math.max(1,Math.pow(a.dy-b.dy,2));
				b.dz = b.dz - swapBZ * a.masse/Math.max(1,Math.pow(a.dz-b.dz,2));
				a.dx = a.dx + swapBX * b.masse/Math.max(1,Math.pow(a.dx-b.dx,2));
				a.dy = a.dy + swapBY * b.masse/Math.max(1,Math.pow(a.dy-b.dy,2));
				a.dz = a.dz + swapBZ * b.masse/Math.max(1,Math.pow(a.dz-b.dz,2));
...
//				}
	...
	public void getKreisGrav2(long it) {
		...
		for (int i = 0; i < it; i++) {
			KreisPunkt a = strukt[(int) (Math.random() * strukt.length)];
			KreisPunkt b = strukt[(int) (Math.random() * strukt.length)];
			if(Math.pow(a.dx-b.dx,2)<1||Math.pow(a.dy-b.dy,2)<1||Math.pow(a.dz-b.dz,2)<1) {
			// if (a == null || b == null) {
			// konstStrukt(1000, 1000, 100, 1000);
			// }
			// a.eigenbeschleunigung(b);
			...
			if (notme) {
				...
				a.posX = a.posX - swapAX * b.masse/Math.max(1,Math.pow(a.posX-b.posX,2));;
				a.posY = a.posY - swapAY * b.masse/Math.max(1,Math.pow(a.posY-b.posY,2));;
				a.posZ = a.posZ - swapAZ * b.masse/Math.max(1,Math.pow(a.posZ-b.posZ,2));;
				b.posX = b.posX + swapAX * a.masse/Math.max(1,Math.pow(a.posX-b.posX,2));;
				b.posY = b.posY + swapAY * a.masse/Math.max(1,Math.pow(a.posY-b.posY,2));;
				b.posZ = b.posZ + swapAZ * a.masse/Math.max(1,Math.pow(a.posZ-b.posZ,2));;

				b.posX = b.posX - swapBX * a.masse/Math.max(1,Math.pow(a.posX-b.posX,2));;
				b.posY = b.posY - swapBY * a.masse/Math.max(1,Math.pow(a.posY-b.posY,2));;
				b.posZ = b.posZ - swapBZ * a.masse/Math.max(1,Math.pow(a.posZ-b.posZ,2));;
				a.posX = a.posX + swapBX * b.masse/Math.max(1,Math.pow(a.posX-b.posX,2));;
				a.posY = a.posY + swapBY * b.masse/Math.max(1,Math.pow(a.posY-b.posY,2));;
				a.posZ = a.posZ + swapBZ * b.masse/Math.max(1,Math.pow(a.posZ-b.posZ,2));;
				
				double faktor=0.002;
				
				a.dx = a.dx - swapAX * b.masse/Math.max(1,Math.pow(a.dx-b.dx,2))*faktor;
				a.dy = a.dy - swapAY * b.masse/Math.max(1,Math.pow(a.dy-b.dy,2))*faktor;
				a.dz = a.dz - swapAZ * b.masse/Math.max(1,Math.pow(a.dz-b.dz,2))*faktor;
				b.dx = b.dx + swapAX * a.masse/Math.max(1,Math.pow(a.dx-b.dx,2))*faktor;
				b.dy = b.dy + swapAY * a.masse/Math.max(1,Math.pow(a.dy-b.dy,2))*faktor;
				b.dz = b.dz + swapAZ * a.masse/Math.max(1,Math.pow(a.dz-b.dz,2))*faktor;

				b.dx = b.dx - swapBX * a.masse/Math.max(1,Math.pow(a.dx-b.dx,2))*faktor;
				b.dy = b.dy - swapBY * a.masse/Math.max(1,Math.pow(a.dy-b.dy,2))*faktor;
				b.dz = b.dz - swapBZ * a.masse/Math.max(1,Math.pow(a.dz-b.dz,2))*faktor;
				a.dx = a.dx + swapBX * b.masse/Math.max(1,Math.pow(a.dx-b.dx,2))*faktor;
				a.dy = a.dy + swapBY * b.masse/Math.max(1,Math.pow(a.dy-b.dy,2))*faktor;
				a.dz = a.dz + swapBZ * b.masse/Math.max(1,Math.pow(a.dz-b.dz,2))*faktor;
	...
	KreisPunkt[] strukt = new KreisPunkt[500];
	...
	public void drawKreisePunkte(double zoom) {
		...
		for (int i = 0; i < strukt.length; i++) {
			...
			// if (kp.fern)
			// kreis.sendBackward();
			add(kreis, (kp.posX) * zoom + kp.posXMax / 2, kp.posY * zoom + 150);
...
//		raumzeitneu(t);
	...
	public static void main(String[] args) {
		new MassekugelnNext2().start(args);
	}
