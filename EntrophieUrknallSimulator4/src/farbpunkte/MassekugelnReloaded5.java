...
public class MassekugelnReloaded5 extends GraphicsProgram {
...
//		if(ButtonCount==null)ButtonCount=new double[1][15];
		...
		KreisPunkt chosKreis =strukt[(int)
		                             ButtonCount[1][1]];
...
//				maxMass = 1;
			...
			int b = 6;
			double masse = 0; 
			if(i<moveKreis/1000.0) {masse = maxMass*Math.random();size=15;}
			else if (i<moveKreis/100.0) {masse = maxMass/10.0*Math.random();size=12;}
			else if (i<moveKreis/10.0) {masse = maxMass/100.0*Math.random();size=9;}
			else {masse = maxMass/1000.0*Math.random();size=6;}
...
//		drawKreisePunkte2(0.2);
	...
	public static void main(String[] args) {
		new MassekugelnReloaded5().start(args);
	}
