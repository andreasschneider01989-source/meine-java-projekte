...
public class Massekugeln extends GraphicsProgram {

	double[][] objekte = new double[100][16]; // Atribute 0-16
	GOval[] kreise = new GOval[100];

	public void initt() {
		for (int i = 0; i < objekte.length; i++) {
			objekte[i][0] = Math.random() * 255;
			
			for (int j = 1; j < 16; j++) {
				objekte[i][j] = Math.random() * 35500;
			}
		}
	}
		...
		while (System.currentTimeMillis() - time < 100) {
			for (int kreisNrA = 0; kreisNrA < kreise.length; kreisNrA++) {
				for (int kreisNrB = kreisNrA + 1; kreisNrB < kreise.length; kreisNrB++) {
					...
					double dist = 0.0;
					int len = objekte[0].length;
					while (dist < 1.0 && len > 0) {
						len--;
						dist = dist + (objekte[kreisNrA][len] - objekte[j][len])
								* (objekte[kreisNrA][len] - objekte[j][len]);
					}
					// double MA = objekte[kreisNrA][0];
					// double MB = objekte[j][0];
					//
					// double Dist1 = (objekte[kreisNrA][1] - objekte[j][1]) * (objekte[kreisNrA][1]
					// - objekte[j][1]);
					// double Dist2 = (objekte[kreisNrA][2] - objekte[j][2]) * (objekte[kreisNrA][2]
					// - objekte[j][2]);
					// double Dist3 = (objekte[kreisNrA][3] - objekte[j][3]) * (objekte[kreisNrA][3]
					// - objekte[j][3]);
					// double dist = Dist1 + Dist2 + Dist3;
					int i = 0;
					while (i < objekte[0].length - 1) {
						i++;
						...
						if (objekte[kreisNrA][i] < swapA)
							swapA = objekte[kreisNrA][i];

						objekte[kreisNrA][i] -= swapA;
						objekte[kreisNrB][i] += swapA;

						if (objekte[kreisNrB][i] < swapB)
							swapB = objekte[kreisNrB][i];
	...
	public void drawKreise() {
		...
		for (int i = 0; i < kreise.length; i++) {
			...
			double r = objekte[i][4] / objekte[i][0];
			double g = objekte[i][5] / objekte[i][0];
			double b = objekte[i][6] / objekte[i][0];
			...
			;
			double x = objekte[i][3] / objekte[i][0];
			;
			double y = objekte[i][1] / objekte[i][0];
			;
			double z = objekte[i][2] / objekte[i][0];
			;
			...
			kreise[i] = new GOval((int) Math.min(25, (objekte[i][0] * 0.01 * objekte[i][0] * 0.01)),
					(int) Math.min(25, (objekte[i][0] * 0.01 * objekte[i][0] * 0.01)));
