...
public class MassekugelnNextSave6 extends GraphicsProgram {
	...
	public void getKreisGrav2(long it) {
		int dismul = 5;
		for (int i = 0; i < it; i++) {
			KreisPunkt a = strukt[Math.max(0, (int)(Math.random() * Math.min(strukt.length,moveKreis)))];
			KreisPunkt b = strukt[Math.max(0, (int)(Math.random() * Math.min(strukt.length,moveKreis)))];
	...
	void drawButtons(int x, int y, int sizeX, int sizeY) {
		...
		for (int breite = 0; breite < sizeX; breite++) {
			for (int hohe = 0; hohe < sizeY; hohe++) {
				...
				feld.setFilled(true);
				feld.setFillColor(new Color(55, 205, 55));
				feld.setColor(new Color(0, 0, 0));
...
//		if(ButtonCount==null)ButtonCount=new double[1][15];
	...
	public static void main(String[] args) {
		new MassekugelnNextSave6().start(args);
	}
